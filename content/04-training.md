...
> 💡 比喻：Pretrain 是把模型关进图书馆，只做一件事——**遮住每句话的最后一个字，猜它是什么**。猜错了就微调体内的 6400 万个数字，让下次猜得更准。把 127 万句话（mini 数据集）猜完一遍，模型就"背下了语言的规律"。就这么一个看似弱智的游戏，是一切能力的地基。
...
> 💡 **比喻 · 梯度累积**：batch 32 太小吃不饱 GPU，但直接开 batch 256 显存爆。于是攒 8 次 32 的梯度一起算总账——"月薪日结攒着月底一次大采购"，数学上等价于 batch 256。
>
> **比喻 · 梯度裁剪**：某批数据让梯度爆冲（范数突然巨大），裁剪把梯度向量"长度"按比例缩短到上限内——高速路上限速，防一个急弯翻车。
>
> **比喻 · fp16 + GradScaler**：float16 是"省墨模式"（显存/带宽减半，T4 快），但小数值梯度会下溢变成 0（墨太淡看不见）。GradScaler 的策略：**先给 loss 乘个大数放大梯度 → 更新前再除回来**——先放大镜找蚂蚁，再按比例还原。
