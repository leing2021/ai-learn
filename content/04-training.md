...
## 4.8 LoRA：263MB 底座不动，1.6MB 改身份

实测（Mac M4 CPU，40min）：冻结 K3 的 SFT 权重，只训插入的低秩矩阵（0.61% 参数），把模型自我认知改写为自定义身份。

**两个关键数字**：

| 项 | 值 |
|---|---|
| LoRA 可训参数 | 0.39M / 64.3M = **0.61%** |
| 适配器文件 | **1.6MB**（对比全参 263MB） |

**验收**：520 步后，"你是谁"从发散回答变成 ✅ 自定义身份；"你和 openai 什么关系"从幻觉变成 ✅ 明确划界。

**代价同样清晰**：纯身份 100 条训练后，**流畅度受损**——出现复读、后半句崩坏。64M 小模型容量太小，新身份模式挤占了语言模式。

> 💡 **比喻**：LoRA 像给书贴便利贴——原书一字不改，新知识写在几张小纸条上夹进去。但书的空白处就那么大，便利贴贴太多，正文反而看不清。

> 🔬 **实战判读**：LoRA 欠拟合的信号是 loss 降不动 + 输出混乱（v1：130 步 loss 3.3）；过拟合信号是身份答对但通用问题变差（v2）。官方忠告"混入通用数据防遗忘"实测成立。
